...
public class MoveNameRetriever : IMoveParser
{
    ...
    public ParsingResult Parse(MoveContextBuilder moveContextBuilder, IEnumerable<string> args)
    {
        ...
        {
            return new ParsingResult.FailureCurrentGoToNextParserWithMessage("first part of command name \"file move\" not found");
        }

        if (listCommandLineArguments.Find(x => x == commandNameSecondPart) != commandNameSecondPart)
        {
            return new ParsingResult.FailureCurrentGoToNextParserWithMessage("second part of command name \"file move\" not found");
        }
